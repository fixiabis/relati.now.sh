...
const Play = () => {
  ...
  const saveGame = () => {
    const placementRecordJSONText = JSON.stringify(game.placementRecords);
    const fileType = "text/json";
    const file = new Blob([placementRecordJSONText], { type: fileType });
    const fileUrl = URL.createObjectURL(file);
    const nowTime = Date.now();
    const fileName = `relati-record-${nowTime}.json`;
